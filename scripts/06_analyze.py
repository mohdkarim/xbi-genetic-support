...
score of its most advanced scoreable program (lead program) is > threshold.
Primary threshold = 0.10; sensitivity at 0.05, 0.15, 0.20.
...
def portfolio_stats(
    ...
    nongs_ci = bootstrap_ci(nongs_returns, n_iter=n_iter, seed=42+1)
...
# ── Primary analysis: GS (lead program score > 0.10) ────────────────────────
...
stats_primary = portfolio_stats(
    gs_returns, nongs_returns,
    "XBI-G (lead program score > 0.10)",
    "XBI-nonG",
    n_iter=2000, seed=42
)

# ── Mendelian-only GS ────────────────────────────────────────────────────────
mend_mask    = master["mendelian_only_gs"].fillna(False).astype(bool)
...
stats_mend = portfolio_stats(
    ...
    "XBI-G-Mendelian (lead program, Mendelian-only, zero look-ahead)",
    "XBI-nonG-Mendelian",
    n_iter=2000, seed=42
)

# ── Sensitivity at multiple score thresholds ─────────────────────────────────
print("[3/4] Sensitivity analysis across score thresholds...")
...
for thresh_col, thresh_label in [
    ...
    # is_gs is the primary; is_gs_at_0_1 == is_gs but use is_gs for 0.10
    if thresh_col == "is_gs":
        mask = gs_mask
    else:
...
print("[4/4] Sub-group analysis...")

is_onco = master["is_oncology_primary"].fillna(False).astype(bool)

onco_gs   = master.loc[is_onco & gs_mask,   "return_total_pct"].values
onco_ngs  = master.loc[is_onco & nongs_mask, "return_total_pct"].values
nonog_gs  = master.loc[~is_onco & gs_mask,  "return_total_pct"].values
nonog_ngs = master.loc[~is_onco & nongs_mask,"return_total_pct"].values

subgroup = {
    "oncology_gs_mean_pct":      round(float(np.mean(onco_gs)),  2) if len(onco_gs)  > 0 else None,
    "oncology_nongs_mean_pct":   round(float(np.mean(onco_ngs)), 2) if len(onco_ngs) > 0 else None,
    "nonog_gs_mean_pct":         round(float(np.mean(nonog_gs)), 2) if len(nonog_gs) > 0 else None,
    "nonog_nongs_mean_pct":      round(float(np.mean(nonog_ngs)),2) if len(nonog_ngs)> 0 else None,
...
for label, st in [
    ("=== GS (lead program score > 0.10) ===", stats_primary),
    ("=== Mendelian-only GS (zero look-ahead) ===", stats_mend),
]:
    print(label)
    print(f"  {st['gs_label']:<55}  n={st['n_gs']:>3}")
    ...
    print(f"  {st['nongs_label']:<55}  n={st['n_nongs']:>3}")
...
for th in ["0.10", "0.50", "0.80", "0.95"]:
    ...
    marker = " *" if th == "0.10" else ""
    print(f"  {th:<10} {sv['n_gs']:>6} {sv['n_nongs']:>8} "
          f"{gs_mean_str} {ngs_mean_str} "
          f"{alpha_str} {ci_lo:>12} {ci_hi:>10}{marker}")

print()
print("=== Oncology vs Non-Oncology sub-groups ===")
sg = subgroup
onco_gs_str = f"mean={sg['oncology_gs_mean_pct']:+.1f}%" if sg['oncology_gs_mean_pct'] is not None else "n/a"
onco_ngs_str = f"mean={sg['oncology_nongs_mean_pct']:+.1f}%" if sg['oncology_nongs_mean_pct'] is not None else "n/a"
nonog_gs_str = f"mean={sg['nonog_gs_mean_pct']:+.1f}%" if sg['nonog_gs_mean_pct'] is not None else "n/a"
nonog_ngs_str = f"mean={sg['nonog_nongs_mean_pct']:+.1f}%" if sg['nonog_nongs_mean_pct'] is not None else "n/a"
print(f"  Oncology:    GS n={sg['n_oncology_gs']}, {onco_gs_str} | "
      f"non-GS n={sg['n_oncology_nongs']}, {onco_ngs_str}")
print(f"  Non-onco:    GS n={sg['n_nonog_gs']}, {nonog_gs_str} | "
      f"non-GS n={sg['n_nonog_nongs']}, {nonog_ngs_str}")

# ── Per-company returns (for transparency) ──────────────────────────────────
print("\n  Per-company returns by GS classification...")

gs_companies = master[gs_mask][["ticker","company","return_total_pct","outcome","lead_score","lead_phase"]].sort_values("return_total_pct", ascending=False)
nongs_companies = master[nongs_mask][["ticker","company","return_total_pct","outcome"]].sort_values("return_total_pct", ascending=False)

print("\n  GS portfolio top 10 & bottom 5:")
print(gs_companies.head(10).to_string(index=False))
print("  ...")
print(gs_companies.tail(5).to_string(index=False))

print("\n  non-GS portfolio top 10 & bottom 5:")
print(nongs_companies.head(10).to_string(index=False))
print("  ...")
print(nongs_companies.tail(5).to_string(index=False))

# Append company-level data to results
